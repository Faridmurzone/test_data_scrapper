...
foreach($controlador as $cat => $link) {
	...
	while($prod->find('h2[class=prodNom]',0)) {
		...
		$query = $conn->query("SELECT * FROM listado_productos WHERE title LIKE '%$titulo%' AND precio_lista LIKE '%$precio_lista%' AND date LIKE '%$yesterday%' AND retailer LIKE '%RIBEIRO%';");
